...
def autocomplete():
    ...
    if request.method == 'GET':  # a query has been submitted
        ...
        if prefix is not None:
            ...
            if type == "queries":
                print("======Query=======")
                search_response = response = opensearch.search(body=createAutoCompleteQuery(prefix), index="bbuy_queries")
            else:
                print("======Instant Request=======")
                search_response = response = opensearch.search(body=createAutoCompleteQuery(prefix), index="bbuy_products")
...
def createAutoCompleteQuery(prefix):
    return {
        "suggest": {
            "autocomplete":{
                "prefix": prefix,
